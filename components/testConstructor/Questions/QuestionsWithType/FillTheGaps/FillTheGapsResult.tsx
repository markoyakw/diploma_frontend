...
const FillTheGapsResult: React.FC<FillTheGapsTextResultProps> = ({ question }) => {
    ...
    const getTextResult = () => {
        if (question.answers.length === 0) return <StyledText>Додайте текстові або поля для заповнення</StyledText>
        return question.answers.map((answer, answerNumber) => getNotBreakingLineAnswerAndNumber(answer, answerNumber))
    }
