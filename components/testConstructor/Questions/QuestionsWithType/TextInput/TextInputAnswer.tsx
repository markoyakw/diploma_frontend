...
const TextInputAnswer: React.FC<{ answer: ITextInputAnswer, questionId: number, answerId: number, answersLength: number, _questionId: number }> =
    ({ answer, questionId, answerId, answersLength, _questionId }) => {

        const { addRules, validateField, validateAllFields, unsubscribeRule, validationErrors } = useValidation()
        const dispatch = useAppDispatch()
        const fieldName = `input_question_${_questionId}_answer_${answer._id}`
